...
import {
  FIXED_DT,
  WEAPON_HEAT_PER_SHOT, WEAPON_OVERHEAT_PENALTY, WEAPON_COOL_RATE,
} from '@core/Constants';
import weaponsData from '@data/weapons.json';
import type { WeaponDefData } from '@data/types';

export type WeaponDef = WeaponDefData;

export const WEAPONS: Record<string, WeaponDef> = weaponsData as Record<string, WeaponDef>;
...
export class WeaponSystem {
  ...
  heat = 0;            // 0–1, exposed for HUD
  overheated = false;  // locked out when true

  // Drone support state
  private drones: Drone[] = [];
  private droneOrbitRadius = 40;

  constructor(
    private projPool: ObjectPool<Projectile>,
    private events: EventBus,
    private vfx: VFXSystem,
    private upgrades: UpgradeSystem,
  ) {}

  update(input: InputFrame, player: Player, enemies: readonly EnemyBase[]): void {
   ...
   */
  chainShock(x: number, y: number, _damage: number, enemies: readonly EnemyBase[]): EnemyBase[] {
  ...
  private updateDrones(player: Player, enemies: readonly EnemyBase[], weapon: WeaponDef): void {
