...
interface TutorialStep {
  ...
  text: string;
  subtext: string;
  triggerDepth: number;     // Show at this depth
  dismissOnAction?: string; // Dismiss when player performs this action
  duration: number;         // Max display time (seconds)
}
...
export class TutorialSystem {
  ...
  private readonly FADE_OUT = 0.3;

  constructor(events: EventBus, isFirstRun: boolean) {
    this.steps = [...STEPS];
  ...
  notifyMove(moveX: number): void {
    if (Math.abs(moveX) > 0.3) this.completedActions.add('move');
  }
  ...
  update(dt: number): void {
    ...
    if (this.depth < step.triggerDepth) return;
  ...
  private renderHint(ctx: CanvasRenderingContext2D, step: TutorialStep | undefined, alpha: number): void {
