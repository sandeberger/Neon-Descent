...
import { CANVAS_W, CANVAS_H, CHUNK_HEIGHT, CHUNK_COLS, CHUNK_ROWS, TILE_SIZE, MAX_ENEMIES, MAX_PLAYER_PROJ, MAX_ENEMY_PROJ, MAX_PICKUPS, PICKUP_MAGNET_RANGE, STOMP_BOUNCE, COMBO_DECAY_TIME, } from '@core/Constants';
import { TileType } from './ChunkTemplate';
import { RUN_UPGRADES } from '@systems/UpgradeSystem';
export class World {
    constructor(seed, restoreState) {
        this.enemies = [];
        this.chunks = [];
        this.nextChunkY = 0;
        // In-run shop trigger
        this.chunksSinceUpgrade = 0;
        this.UPGRADE_INTERVAL = 12;
        ...
        this.projPool = new ObjectPool(() => new Projectile(), (p) => p.reset(), (p) => p.active, MAX_PLAYER_PROJ);
        this.enemyProjPool = new ObjectPool(() => new Projectile(), (p) => p.reset(), (p) => p.active, MAX_ENEMY_PROJ);
        this.pickupPool = new ObjectPool(() => new Pickup(), (p) => p.reset(), (p) => p.active, MAX_PICKUPS);
    ...
    executeEmpBlast() {
        ...
        for (const enemy of this.enemies) {
            if (!enemy.active)
                continue;
            // Check if on-screen
            if (enemy.y < this.camera.top - 50 || enemy.y > this.camera.bottom + 50)
                continue;
    ...
    resolveTileCollisions() {
        ...
        for (const chunk of this.chunks) {
            ...
            if (chunk.worldY > p.y + CANVAS_H || chunk.bottom < p.y - CANVAS_H)
                continue;
    ...
    resolveEntityCollisions() {
        ...
        if (this.player.state !== 'DEAD') {
            for (const enemy of this.enemies) {
                if (!enemy.active)
                    continue;
                if (!aabbOverlap(pb, enemy.getBounds()))
                    continue;
    ...
    resolveProjectileTiles() {
        for (const proj of this.projPool.active) {
            if (!proj.active || proj.owner !== 'player')
                continue;
            for (const chunk of this.chunks) {
                if (chunk.worldY > proj.y + 30 || chunk.bottom < proj.y - 30)
                    continue;
    ...
    spawnEnemy(x, y, id) {
        ...
        switch (id) {
            ...
            case 'parasite_cloud':
                e = new ParasiteCloud();
                break;
            case 'sentinel_miniboss':
                e = new SentinelMiniboss();
                break;
            case 'rail_sentinel':
                e = new RailSentinel();
                break;
    ...
    processEnemyShots() {
        for (const enemy of this.enemies) {
            if (!enemy.active || enemy.pendingShots.length === 0)
                continue;
            for (const shot of enemy.pendingShots) {
                if (this.enemyProjPool.activeCount >= MAX_ENEMY_PROJ)
                    break;
    ...
    resolveBeamDamage() {
        ...
        const pb = this.player.getBounds();
        for (const enemy of this.enemies) {
            if (!enemy.active || !(enemy instanceof RailSentinel))
                continue;
            if (!enemy.beamActive)
                continue;
    ...
    serialize() {
        const upgradeIds = this.upgrades.getActiveUpgrades()
            .flatMap(({ def, stacks }) => Array(stacks).fill(def.id));
        return {
            seed: this.seed,
