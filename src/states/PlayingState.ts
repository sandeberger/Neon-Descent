...
export class PlayingState implements GameState {
  ...
  onEnter(prev: string | null): void {
    ...
    if (this.game.pendingLoadoutPerk) {
      ...
      if (perkDef) this.world.upgrades.apply(perkDef);
  ...
  private bindPause(): void {
    ...
    this.pauseTouchListener = (e: Event) => {
  ...
  private triggerPause(): void {
    this.resuming = true;
    const paused = (this.game.states as any).states.get('PAUSED') as PausedState | undefined;
    if (paused) paused.captureFrame();
  ...
  private triggerShop(): void {
    this.resuming = true;
    const shop = (this.game.states as any).states.get('RUN_SHOP') as RunShopState | undefined;
    if (shop) shop.captureFrame();
