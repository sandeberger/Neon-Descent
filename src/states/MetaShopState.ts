...
import { META_UPGRADES, type MetaUpgrade } from '@meta/MetaProgression';
import { CANVAS_W, CANVAS_H } from '@core/Constants';

export class MetaShopState implements GameState {
  readonly name = 'META_SHOP';
  ...
  private tryPurchase(upgrade: MetaUpgrade): void {
  ...
  render(_alpha: number): void {
    ...
    ctx.fillText(
      `RUNS: ${this.meta.totalRuns}  |  BEST: ${Math.floor(this.meta.bestScore).toLocaleString()}  |  ${Math.floor(this.meta.bestDepth / 10)}m`,
      CANVAS_W / 2, 85,
    );
