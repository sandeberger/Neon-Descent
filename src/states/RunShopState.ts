...
import type { UpgradeDef } from '@systems/UpgradeSystem';
import { getRarityColor } from '@systems/UpgradeSystem';
import { CANVAS_W, CANVAS_H } from '@core/Constants';

export class RunShopState implements GameState {
  readonly name = 'RUN_SHOP';
  ...
  private frameSnapshot: ImageData | null = null;

  constructor(private game: Game) {}

  /** Call before transitioning to capture the current frame */
  captureFrame(): void {
    this.frameSnapshot = this.game.ctx.getImageData(
  ...
  onEnter(): void {
    ...
    const handleTouch = (e: Event) => {
      e.preventDefault();
      if (this.fadeIn < 0.3) return; // ignore during fade-in
    ...
    (this as any)._cleanupKey = handleKey;
  }
  ...
  private getPlayingState(): PlayingState | null {
    return (this.game.states as any).states?.get('PLAYING') as PlayingState | null;
  }
  ...
  render(_alpha: number): void {
    ...
    if (playing?.world) {
      ...
      ctx.font = '9px monospace';
      ctx.fillText(
        `DEPTH ${Math.floor(playing.world.scoring.depth / 10)}m`,
        CANVAS_W / 2, 95,
      );
    }
    ...
    for (let i = 0; i < this.choices.length; i++) {
      ...
      if (existing) {
        ...
        ctx.fillText(
          `STACK ${existing.stacks + 1}/${upgrade.maxStacks}`,
          cardPadX + cardW - 10, cardY + cardHeight - 14,
        );
