...
const WEAPON_DEFS: WeaponDef[] = [
  { id: 'balanced_auto', name: 'Pulse Rifle',    color: (weaponsData as any).balanced_auto.color, glowColor: (weaponsData as any).balanced_auto.glowColor, unlockRuns: 0 },
  { id: 'shotgun_burst', name: 'Scatter Cannon', color: (weaponsData as any).shotgun_burst.color, glowColor: (weaponsData as any).shotgun_burst.glowColor, unlockRuns: 3 },
  { id: 'beam_cutter',   name: 'Beam Cutter',    color: (weaponsData as any).beam_cutter.color,   glowColor: (weaponsData as any).beam_cutter.glowColor,   unlockRuns: 5 },
  { id: 'ricochet',      name: 'Ricochet',       color: (weaponsData as any).ricochet.color,      glowColor: (weaponsData as any).ricochet.glowColor,      unlockRuns: 8 },
  { id: 'chain_shock',   name: 'Chain Shock',    color: (weaponsData as any).chain_shock.color,    glowColor: (weaponsData as any).chain_shock.glowColor,    unlockRuns: 12 },
  { id: 'acid_scatter',  name: 'Acid Scatter',   color: (weaponsData as any).acid_scatter.color,   glowColor: (weaponsData as any).acid_scatter.glowColor,   unlockRuns: 15 },
  { id: 'drone_support', name: 'Drone Support',  color: (weaponsData as any).drone_support.color,  glowColor: (weaponsData as any).drone_support.glowColor,  unlockRuns: 20 },
];
...
export class LoadoutState implements GameState {
  ...
  private selectedPerkIndex = -1;          // -1 = none
  private availablePerks: PerkDef[] = [];

  /* visual */
  private pulsePhase = 0;
  private scrollOffset = 0;               // horizontal weapon scroll
  private targetScrollOffset = 0;

  /* layout constants */
  private readonly CARD_W = 88;
  private readonly CARD_H = 100;
  private readonly CARD_GAP = 10;
  private readonly WEAPON_ROW_Y = 130;
  private readonly PERK_ROW_Y = 380;
  private readonly PERK_CARD_W = 100;
  private readonly PERK_CARD_H = 80;
  private readonly PERK_GAP = 10;
  private readonly START_BTN_Y = 560;

  /* input bookkeeping */
  private _cleanupTouch: ((e: Event) => void) | null = null;
  private _cleanupKey: ((e: KeyboardEvent) => void) | null = null;
  ...
  onEnter(_prev: string | null): void {
    this.pulsePhase = 0;
  ...
  private isWeaponUnlocked(idx: number): boolean {
    const w = WEAPON_DEFS[idx];
    return w ? this.game.meta.totalRuns >= w.unlockRuns : false;
  }

  private handleWeaponTap(tapX: number): void {
  ...
  private handlePerkTap(tapX: number): void {
  ...
  private selectPerk(idx: number): void {
  ...
  private moveWeaponSelection(dir: number): void {
    let next = this.selectedWeaponIndex + dir;
    // Wrap
    if (next < 0) next = WEAPON_DEFS.length - 1;
    if (next >= WEAPON_DEFS.length) next = 0;
  ...
  private tryStart(): void {
    if (!this.isWeaponUnlocked(this.selectedWeaponIndex)) return;
  ...
  private renderWeaponCards(ctx: CanvasRenderingContext2D): void {
    const totalW = WEAPON_DEFS.length * (this.CARD_W + this.CARD_GAP) - this.CARD_GAP;
    const startX = (CANVAS_W - totalW) / 2 + this.scrollOffset;
  ...
  private renderPerkCards(ctx: CanvasRenderingContext2D): void {
  ...
  private renderStartButton(ctx: CanvasRenderingContext2D): void {
    ...
    if (canStart) {
      ...
      ctx.lineWidth = 2;
      // Top-left
      ctx.beginPath(); ctx.moveTo(bx, by + accLen); ctx.lineTo(bx, by); ctx.lineTo(bx + accLen, by); ctx.stroke();
      // Top-right
      ctx.beginPath(); ctx.moveTo(bx + btnW - accLen, by); ctx.lineTo(bx + btnW, by); ctx.lineTo(bx + btnW, by + accLen); ctx.stroke();
      // Bottom-left
      ctx.beginPath(); ctx.moveTo(bx, by + btnH - accLen); ctx.lineTo(bx, by + btnH); ctx.lineTo(bx + accLen, by + btnH); ctx.stroke();
      // Bottom-right
      ctx.beginPath(); ctx.moveTo(bx + btnW - accLen, by + btnH); ctx.lineTo(bx + btnW, by + btnH); ctx.lineTo(bx + btnW, by + btnH - accLen); ctx.stroke();
