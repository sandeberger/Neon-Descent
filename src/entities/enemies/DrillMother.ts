...
export class DrillMother extends EnemyBase {
  ...
  onUpdate(dt: number, playerX: number, _playerY: number): void {
    ...
    if (this.x < 50) { this.x = 50; this.driftDir = 1; }
    if (this.x > 310) { this.x = 310; this.driftDir = -1; }
  ...
  private getChargeCooldown(): number {
    switch (this.phase) {
      case 1: return 5.0;
      case 2: return 4.0;
      case 3: return 3.0;
  ...
  private fire(playerX: number): void {
    switch (this.phase) {
      ...
      case 3: {
        ...
        this.pendingShots.push(
          { x: this.x, y: this.y, vx: -120, vy: 120, damage: this.damage },
          { x: this.x, y: this.y, vx: 120, vy: 120, damage: this.damage },
        );
        break;
      }
  ...
  private updateCharge(dt: number): void {
    ...
    // Bounce off walls
    if (this.x < 30) { this.x = 30; this.chargeVx = -this.chargeVx * 0.5; }
    if (this.x > 330) { this.x = 330; this.chargeVx = -this.chargeVx * 0.5; }
