...
export class Mirror extends EnemyBase {
  ...
  private fireTimer = 0;
  private readonly FIRE_INTERVAL = 2.0;
  private readonly BULLET_SPEED = 100;
  private shieldSide: -1 | 1 = 1;
  private pulsePhase = 0;

  /** Shield faces the player — blocks projectiles from that side */
  isShielded(projectileX: number): boolean {
  ...
  onUpdate(dt: number, playerX: number, playerY: number): void {
    ...
    switch (this.state) {
      ...
      case 'IDLE': {
        ...
        // Clamp
        if (this.x < 25) { this.x = 25; this.driftDir = 1; }
        if (this.x > 335) { this.x = 335; this.driftDir = -1; }
  ...
  private fireAtPlayer(playerX: number, playerY: number): void {
