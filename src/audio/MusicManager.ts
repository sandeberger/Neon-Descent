...
export class MusicManager {
  ...
  private layerTargets = [1, 0, 0, 0]; // current target volumes
  ...
  start(ctx: AudioContext, outputNode: GainNode): void {
    this.ctx = ctx;
    this.running = true;
  ...
  updateLayers(comboTier: number, nearDeath: boolean): void {
    const targets = [
      1,                                    // Layer 0: always
  ...
  private scheduleBeat(time: number, beat: number): void {
  ...
  private schedulePercussion(time: number, beat: number): void {
    const ctx = this.ctx!;
  ...
  private scheduleMelody(time: number, beat: number): void {
    const ctx = this.ctx!;
  ...
  private scheduleIntensity(time: number, beat: number): void {
    const ctx = this.ctx!;
